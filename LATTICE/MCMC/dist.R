...
proc.crr<-function(Z,Z0,k=2){
  ...
  return(t(t(Z0)%*%Z%*%solve(Ahalf)%*%t(Z))) }
...
lpY<-function(Y,Z=0,alpha){
  ...
  diag(lpg)<-0
  return(sum( Y*lpg - log( 1+exp(lpg) ) ) + dim(Y)[1]*log(2))
}
...
Z.up<-function(Y,Z,alpha,zdelta,mu.z=0,sd.z=10){
  ...
  if( runif(1)>exp(hr) ){Znew<-Z
    lnew<-lold }
  
  return(list(Z=Znew,lik=lnew))}
...
alpha.up<-function(Y,Z,alpha,adelta,a.a=1,a.b=1){
  ...
  if(runif(1)>hr){ alphanew<-alpha
    lnew<-lold }
  
  return(list(alpha=alphanew,lik=lnew))  }
